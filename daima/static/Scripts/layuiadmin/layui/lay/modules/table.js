...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
    ...
    Class.prototype.setInit = function (type) {
        ...
        options.clientWidth =  function () { //获取容器宽度
    ...
    Class.prototype.pullData = function (curr) {
        ...
        if (options.remoteSort) {//marje
            if (typeof options.initSort === 'object' && options.initSort.field != "") {
                var sort = options.initSort.field.split(',');
                var order = options.initSort.type.split(',');
                layui.each(sort, function (i, item) {
                    that.remoteSort(sort[i], order[i]);
                })
            }
        }
        ...
        if (options.url) { //Ajax请求
            ...
            if (options.remoteSort) {//marje
                params["sort"] = options.initSort.field;
            ...
            $.ajax({
                ...
                , success: function (res) {
                    ...
                    if (res[response.statusName] != response.statusCode) {
                        ...
                        that.renderData(res, curr, res[response.countName]);//marje
        ...
        that.eachCols(function (i3, item3) {
            ...
            var td = ['<td data-field="' + field + '" data-key="' + options.index + '-' + item3.key + '" ' + function () {
                ...
                , '<div class="layui-table-cell laytable-cell-' + function () { //返回对应的CSS类标识
                    ...
                    return item3.totalRow ? (data[field] || text) : text;
    ...
    //数据远程排序--marje
    Class.prototype.remoteSort = function (th, type, pull, formEvent) {
        var that = this
            , field
            , res = {}
            , options = that.config
        ...
        try {
            ...
            var elemSort = that.layHeader.find('th .laytable-cell-' + key).find(ELEM_SORT);
            //  that.layHeader.find('th').find(ELEM_SORT).removeAttr('lay-sort'); //清除其它标题排序状态marje
            elemSort.attr('lay-sort', type || null);
            that.layFixed.find('th')
        } catch (e) {
            return hint.error('Table modules: Did not match to field');
        ...
        if (formEvent) {//marje
            ...
            if (options.initSort.field != "") {
                sort = options.initSort.field.split(',');
                order = options.initSort.type.split(',');
            }
            var sortIndex = $.inArray(field, sort);
            if (type == null && sortIndex > -1) {
                sort.splice(sortIndex, 1);
                order.splice(sortIndex, 1);
            } else {
                if (sortIndex > -1) {
                    order[sortIndex] = type;
                } else {
                    sort.push(field);
                    order.push(type);
                }
            ...
            options.initSort.field = sort.join(',');
            options.initSort.type = order.join(',');
    ...
    Class.prototype.events = function () {
        ...
        th.on('click', function (e) {
            ...
            that.remoteSort(othis, type, null, true);//marje
            ...
            layui.stope(e);
            if (index === 0) {
                that.remoteSort(field, 'asc', null, true);//marje
            } else {
                that.remoteSort(field, 'desc', null, true);//marje
            }
        });
